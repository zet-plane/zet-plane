...
│ ContextBuilder│ │  LlmProviderRegistry               │
│ 初始上下文   │ │  SkillRegistry · PromptBuilder      │
│ 构造         │ │                                    │
└──────────────┘ └─────────────────┬──────────────────┘
...
### 4.4 推理层（LlmProviderRegistry + SkillRegistry + PromptBuilder + Agentic Loop）
...
特殊 skill `_base/index.md`（`base: true`）定义 agent 的通用身份和行为规范，始终前置于所有 task-specific skill 之前，无需在 `applicable_tasks` 中列出。

每次 task 执行时，`getSystemPrompt(taskType)` 返回：base skill + 匹配当前 task type 的所有 skill，按顺序拼接。

**PromptBuilderService**

负责将 SkillRegistry 和 OrchestratorContext 组装成 LLM 可用的 prompt：

```
SkillRegistry.getSystemPrompt(taskType) → systemPrompt
OrchestratorContext                     → userMessage（结构化文本）
```

`TaskRunnerService` 只调用 `promptBuilder.build(task, ctx)`，不直接依赖 SkillRegistry。
...
| Task | 模型策略 |
|---|---|
| `embedding` | embedding model（`orchestrator.llm.embeddingModel`），不走 chat，无 agentic loop |
| 其他 task | 查 `orchestrator.llm.taskModels[taskType]`，fallback 到 `default` |

模型通过 `LlmProviderRegistry.getChatModelForTask(taskType)` 按 config 动态路由，不在代码中硬编码。详见 `2026-05-12-llm-provider-abstraction-design.md`。
...
└── orchestrator/                          # Orchestrator 专属 skill 命名空间
    ├── _base/                             # base: true，agent 通用身份，始终前置
    │   └── index.md
    ├── event-anchoring/
    │   └── index.md
...
│
├── llm/                                   # LLM Provider 层（provider 抽象，详见 llm-provider-abstraction-design.md）
│   ├── interfaces.ts                      # IChatProvider / IEmbeddingProvider
│   ├── llm-provider.registry.ts           # 注入 AppConfig，按 task type 路由到对应 provider
│   └── providers/
│       ├── anthropic.provider.ts
│       └── openai-compatible.provider.ts  # 同时实现 IChatProvider + IEmbeddingProvider
│
├── prompt/                                # Prompt 组装层
│   └── prompt-builder.service.ts         # skill → systemPrompt；ctx → userMessage
│
├── agent/                                 # Agentic loop 层（基于 LangGraph）
│   └── agent-graph.ts                     # LangGraph StateGraph 定义：LLM 节点 + tool 节点 + conditional edge
│
├── skill/                                 # Skill 层（system prompt 来源）
│   └── skill-registry.ts                  # 扫描 skills/ 目录；base: true skill 始终前置；按 task type 匹配
│
