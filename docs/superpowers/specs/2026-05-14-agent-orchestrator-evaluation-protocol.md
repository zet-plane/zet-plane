...
// 创建 N1（checkpoint）
const N1 = (await app.inject({ method: 'POST', url: `/projects/${projectId}/nodes`,
  payload: { title: 'MVP 阶段', type: 'scaffold' } })).json()

// 将 N1 设为 checkpoint
await app.inject({ method: 'PATCH', url: `/nodes/${N1.id}`,
  payload: { isCheckpoint: true } })
...
// 创建 composition 边：N2/N3/N4 → N1（注意 fromId → toId 方向）
for (const nId of nodeIds) {
  await app.inject({ method: 'POST', url: `/projects/${projectId}/edges`,
    payload: { fromId: nId, toId: N1.id, type: 'composition', createdBy: 'human' } })
}

// 将子节点状态更新为 completed
for (const nId of nodeIds) {
  await app.inject({ method: 'PATCH', url: `/nodes/${nId}`,
...
| confidence > 0.7 | ✅ / ❌ |
| evidence 中有 sourceType=node 且 sourceId=N1.id | ✅ / ❌ |
...
> **执行流程**: agent 调用 `create_edge` → domain service 检测到环 → 抛出 `DomainServiceError` → agent 收到错误信息 → agent 调用 `notify_human` → 抛出 `WaitingForApprovalSignal` → runtime 将 task.status 置为 `waiting_for_approval`。
