...
### 2.1 多图结构与暂存图（Staging Graph）

一个 Project 可以开展多个流程，每个流程对应一个 Graph；Graph 内部可以嵌套 Sub-Graph（大图包含小图）。因此系统中并不存在"一个项目一张图"的简单对应关系。

**Staging Graph** 是一类特殊用途的 Graph，归属粒度为 **Graph 级别**：每个 Graph 可以有对应的 Staging Graph，用于承载无法立即锚定到该 Graph 具体节点的事件。

**设计动机**：事件进入系统时，Orchestrator 可能需要积累多个相关事件才能判断应挂到哪个节点，单个事件无法立即决策。Staging Graph 提供了结构化的暂存空间——Orchestrator 在其中创建 Growth Node 并用边表达事件关系，待上下文足够后整体迁移至目标 Graph。

**关键约定**：
- Staging Graph 本身是一个普通 Graph，**由 Orchestrator 决定**何时为某个 Graph 创建 Staging Graph 以及如何使用
- 知识条目**必须锚定到某个节点**（任意 Graph 或 Staging Graph 的节点均可）
- 不存在 `nodeId` 为空的知识条目，「无游离知识」约束始终成立
- KE 不感知图的类型（主图 / 暂存图 / 子图），`nodeId` 对 KE 是透明外键
...
> KE 不感知图类型，以下流程由 Agent Orchestrator 全权驱动。

```
外部事件（无法立即锚定到某 Graph 的具体节点）
      │
      ▼
Agent Orchestrator
  判断事件属于哪个 Graph 的范围
      │
      ├── 若该 Graph 尚无 Staging Graph：通过 Graph Engine API 创建
      │
      ▼
  在 Staging Graph 内创建 Growth Node 承载事件
  （相关事件之间用边表达关联关系）
      │
      ▼
  创建 KnowledgeEntry，nodeId → Staging Graph 节点
      │
      ▼
  （持续积累事件，在 Staging Graph 中完善节点与边的关系）
      │
      ▼
  上下文足够，决定迁移
      │
      ├── 在目标 Graph（或 Sub-Graph）中创建 / 定位目标节点
      └── PATCH /entries/:id { nodeId: <目标节点 id> }
                │
                ▼
          knowledge.entry.reanchored 事件
```
