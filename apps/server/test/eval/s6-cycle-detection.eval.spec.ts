...
describe('S-6: Checkpoint Decision Package + Gate-Based Wait', () => {
  let ctx: EvalApp
  ...
  it('P1–P4: checkpoint task creates decision draft and succeeds with decision evidence', async () => {
    ...
    expect(checkpointTaskAfter.status).toBe('succeeded')
    ...
    const insight = parseInsight(checkpointTaskAfter)
    expect(insight?.signalType).toBe('decision')
    expect(insight?.evidence.length).toBeGreaterThanOrEqual(1)

    const decisionEvidence = insight?.evidence.find(e =>
      e.sourceType === 'knowledge_entry' &&
      decisionEntries.some(entry => entry.id === e.sourceId),
    )
    expect(decisionEvidence).toBeDefined()
    ...
    printRecord('S-6', {
      ...
      'decision evidence sourceId': decisionEvidence?.sourceId ?? null,
      'decision entries': decisionTitles,
      'decision entry 数': decisionEntries.length,
      'decision package 完整性': hasDecisionPackage ? 'Y' : 'N',
      'checkpoint node waits via gate': checkpointNode.isCheckpoint && checkpointNode.status === 'blocked',
    })
  }, 180_000)
})
  ...
  const eventTask = await publishAndExecute(ctx, {
    ...
    sourceId: `manual-s6-${Date.now()}`,
      input: withEvalTrace({
        ...
        evalCase: 'S-6',
        testName: 'P1–P4: checkpoint task creates decision draft and succeeds with decision evidence',
