...
describe("aggregateStatus", () => {
	...
	it("seals archived subtrees so descendants do not affect ancestors", () => {
		const g: ProjectGraph = {
			...
			edges: [
				composition("root", "archived", 0),
				composition("archived", "blocked", 1),
			],
		};
	...
	it("seals completed subtrees so descendants do not affect ancestors", () => {
		const g: ProjectGraph = {
			...
			edges: [
				composition("root", "completed", 0),
				composition("completed", "blocked", 1),
			],
		};
	...
	it("ignores duplicate parents targeting a missing child", () => {
		const g: ProjectGraph = {
			...
			edges: [
				composition("root", "missing", 0),
				composition("other", "missing", 1),
			],
		};
	...
	it("ignores cycles among missing composition ids", () => {
		const g: ProjectGraph = {
			nodes: [node("root", "active")],
			edges: [
				composition("missing-a", "missing-b", 0),
				composition("missing-b", "missing-a", 1),
			],
		};
	...
	it("throws when a child has duplicate composition parents", () => {
		const g: ProjectGraph = {
			...
			edges: [
				composition("root", "child", 0),
				composition("other", "child", 1),
			],
		};
	...
	it("throws on composition cycles with the cycle path", () => {
		...
		expect(() => aggregateStatus(g)).toThrowError(
			"Composition cycle detected: a -> b -> c -> a",
		);
	});
});
