...
import { z } from "zod";
import { ApiError, apiCall, type EndpointDef } from "./api-client";
...
describe("apiCall", () => {
	...
	it("returns undefined for 204 endpoints without a response schema", async () => {
		...
		await expect(
			apiCall(endpoint, { params: { id: "edge-1" } }),
	...
	it("allows empty responses to be declared with z.void()", async () => {
		...
		await expect(
			apiCall(endpoint, { params: { id: "project-1" } }),
		).resolves.toBeUndefined();
	});
	...
	it("parses declared error envelopes", async () => {
		vi.spyOn(globalThis, "fetch").mockResolvedValue(
			new Response(
				JSON.stringify({ code: "EDGE_NOT_FOUND", message: "Edge not found" }),
				{
					status: 404,
					headers: { "Content-Type": "application/json" },
				},
		...
		await expect(
			apiCall(endpoint, { params: { id: "missing" } }),
